...
const myParamUser = urlParams.get('username');
...
const url = `https://5f771bbcd5c9cb0016236ac1.mockapi.io/api/user?username=${myParamUser}`
...
async function getdatafromdata(url_api) {
    ...
    if (index == -1) { postdata(url_data) }
        ...
        function addcmt() {
            ...
            comment.innerHTML = `${comment.innerHTML}` + `<p><img style=" font-size: 25pt;height: 50px;width: 50px;" src="${userdata[0].avatar}">${userdata[0].username}: ${cmt.value}</p>`
            putDatas(url_data)
            cmt.value = ""
            // console.dir(cmt)
        }
